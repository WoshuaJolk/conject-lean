...
/-- `Statements.ThetaCliqueSecondMoment.statement`. -/
theorem proof :
    ∀ (V : Type) [Fintype V] [DecidableEq V] [Nonempty V]
      ...
      Commons.thetaClique adj ≤ (m / (m - d)) * (1 + Real.sqrt (d - d ^ 2 / m + c)) := by
  intro V _ _ _ adj A R m d c hm hsymm hirr hA1 hA0 hrow hRdiag hR hc
  exact thetaClique_le ⟨hm, hsymm, hirr, hA1, hA0, hrow, hRdiag, hR, hc⟩

end Submissions.ThetaCliqueSecondMoment.WoshuaJolk
