...
theorem proof :
  ...
  · intro i j hij
    constructor
    · intro he
      have hz : dot4Z (vZ i) (vZ j) = 0 := (orthExact i j hij).mp (by
        -- match the two `let` spellings
        simpa using he)
      have hdot :
          (∑ r, star (v i r) * v j r) = (dot4Z (vZ i) (vZ j) : ℂ) := by
  ...
  · intro i j k l t hij hjk hkl hlt
    ...
    · exact Or.inl (linInd_v i j k l h)
    · exact Or.inr <| Or.inl (linInd_v i j k t h)
    · exact Or.inr <| Or.inr <| Or.inl (linInd_v i j l t h)
    · exact Or.inr <| Or.inr <| Or.inr <| Or.inl (linInd_v i k l t h)
    · exact Or.inr <| Or.inr <| Or.inr <| Or.inr (linInd_v j k l t h)

end Submissions.SpanningOrthRep4C10.Circ10
