...
theorem swapSPS {a b m : ℕ} {A B : Fin m → Finset ℕ} (h : Commons.OneCrossSPS a b m A B) :
...
/-- The mirror peel: one unit off the `a` side costs a factor `b`. -/
theorem mirror_peel {a b m : ℕ} {A B : Fin m → Finset ℕ}
    (h : Commons.OneCrossSPS (a + 1) b m A B) :
    ∃ (m' : ℕ) (A' B' : Fin m' → Finset ℕ),
      Commons.OneCrossSPS a b m' A' B' ∧ m ≤ b * m' + 1 := by
  obtain ⟨m', A', B', hs, hle⟩ := dual_peel (swapSPS h)
  exact ⟨m', B', A', swapSPS hs, hle⟩
...
theorem proof : ∀ (a b m : ℕ) (A B : Fin m → Finset ℕ),
  ...
  fun _ _ _ _ _ h => mirror_peel h

end Submissions.MirrorPeelRecursion.MirrorPeel
