...
theorem proof : ∀ (a b m t : ℕ) (A B : Fin m → Finset ℕ) (T : ℕ → Finset (Fin m)),
    Commons.OneCrossSPS a b m A B →
    (∀ (e : ℕ) (j : Fin m), j ∈ T e ↔ e ∈ B j) →
    (∀ e : ℕ, (T e).card ≤ t) →
    m ≤ a * t + 1 := by
  intro a b m t A B T h hT hdeg
  have hTeq : ∀ e : ℕ, T e = Tset B e := by
    intro e
    ext j
    rw [hT e j, Tset, Finset.mem_filter]
    simp
  refine degree_ceiling h (fun e => ?_)
  rw [← hTeq e]
  exact hdeg e

end Submissions.GroundDegreeCeiling.HubIdentity
