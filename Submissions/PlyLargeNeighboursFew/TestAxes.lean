...
/-- The submitted proof of `Statements.PlyLargeNeighboursFew.statement`. -/
theorem proof :
    ∀ (d n k : ℕ), 1 ≤ d →
      ∀ (x : Fin n → EuclideanSpace ℝ (Fin d)) (r : Fin n → ℝ),
        ...
        ∀ i₀ : Fin n, (∀ i, r i₀ ≤ r i) →
          {i : Fin n | i ≠ i₀ ∧
              ...
              3 * (d : ℝ) * r i₀ ≤ r i}.ncard
            ≤ (2 * d + 1) * k :=
  fun d n k hd x r hr hthin i₀ hmin => large_neighbours_few hd x r hr hthin i₀ hmin

end Submissions.PlyLargeNeighboursFew.TestAxes
