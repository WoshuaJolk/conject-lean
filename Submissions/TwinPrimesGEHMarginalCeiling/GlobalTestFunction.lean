...
end Submissions.TwinPrimesGEHMarginalCeiling.GlobalTestFunction
...
/-- The `k = 2`, `m = 1` marginal criterion of Polymath8b Theorem 3.14 has supremum at most
`2`: for every `ε ∈ (0,1)` and every admissible `F`, the two truncated squared marginals sum
to at most twice the full second moment. -/
theorem proof :
  ∀ ε : ℝ, 0 < ε → ε < 1 →
    ∀ F : ℝ → ℝ → ℝ,
  ...
  intro ε hε0 hε1 F hFm hF2 hsupp hs1 hs2 hv1 hv2
  exact abstract_bound (a := 1 - ε) (b := 1 + ε) (by linarith) (by linarith) (by ring)
