...
/-!
# MUST-FAIL CONTROL. This is deliberately the WRONG theorem.

This module proves `paleyLocTheta p ≤ 3 + √(p/2 + 4c)` where the canonical statement asks for
`2 + √(p/2 + 4c)`.  The claim proved here is strictly weaker and follows from the canonical
one in one line, so it is exactly the kind of near-miss the anti-restatement gate exists to
reject.  It should red with reason `restatement`.  If it were to go green, the gate on
`Statements/PaleyLocSecondMomentUnconditional.lean` would be broken and every verdict
recorded under that label would be worth less.

Everything below the docstring is the real argument, changed only in the final constant.  Three parts.
...
namespace Submissions.PaleyLocSecondMomentUnconditional.Control
...
theorem weakProof :
    ∀ (p : ℕ) [NeZero p] (hp : Nat.Prime p), p % 4 = 1 → 5 < p →
      ∀ (A R : Matrix (Commons.PaleyLocV p) (Commons.PaleyLocV p) ℝ) (c : ℝ),
        ...
        Commons.paleyLocTheta p hp.pos ≤ 3 + Real.sqrt ((p : ℝ) / 2 + 4 * c) := by
  classical
  ...
  calc (m / (m - d)) * (1 + Real.sqrt K) ≤ 2 * (1 + Real.sqrt K) := step1
    ...
    _ ≤ 3 + Real.sqrt (P / 2 + 4 * c) := by
        have := Real.sqrt_le_sqrt hle
        linarith

end Paley

end Submissions.PaleyLocSecondMomentUnconditional.Control
