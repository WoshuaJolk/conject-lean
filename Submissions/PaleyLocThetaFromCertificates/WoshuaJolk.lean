...
theorem proof :
  ...
  intro HA HB ε hε
  obtain ⟨NA, hNA⟩ := HA (ε / 8) (by linarith)
  obtain ⟨NB, hNB⟩ := HB (ε / 4) (by linarith)
  refine ⟨max (max NA NB) (max 6 (max ⌈32 / ε ^ 2⌉₊ ⌈4 / (3 * ε)⌉₊)), ?_⟩
  intro p hp h4 hbig
  haveI : NeZero p := ⟨hp.ne_zero⟩
  have hpNA : NA < p := lt_of_le_of_lt (le_trans (le_max_left _ _) (le_max_left _ _)) hbig
  have hpNB : NB < p := lt_of_le_of_lt (le_trans (le_max_right _ _) (le_max_left _ _)) hbig
  have hp6 : 6 < p := lt_of_le_of_lt (le_trans (le_max_left _ _) (le_max_right _ _)) hbig
  have hp5 : 5 < p := by omega
  have hceil1 : ⌈32 / ε ^ 2⌉₊ < p :=
    lt_of_le_of_lt (le_trans (le_max_left _ _) (le_trans (le_max_right _ _) (le_max_right _ _)))
      hbig
  have hceil2 : ⌈4 / (3 * ε)⌉₊ < p :=
    lt_of_le_of_lt (le_trans (le_max_right _ _) (le_trans (le_max_right _ _) (le_max_right _ _)))
      hbig
  have hP1 : (32 : ℝ) / ε ^ 2 < (p : ℝ) :=
    lt_of_le_of_lt (Nat.le_ceil _) (by exact_mod_cast hceil1)
  have hP2 : (4 : ℝ) / (3 * ε) < (p : ℝ) :=
    lt_of_le_of_lt (Nat.le_ceil _) (by exact_mod_cast hceil2)
  have hPpos : (0 : ℝ) < (p : ℝ) := by positivity
  have hP5 : (5 : ℝ) < (p : ℝ) := by exact_mod_cast hp5
  set s : ℝ := Real.sqrt ((p : ℝ) / 2) with hs
  have hspos : 0 < s := Real.sqrt_pos.mpr (by linarith)
  have hs2 : s ^ 2 = (p : ℝ) / 2 := Real.sq_sqrt (by linarith)
  -- upper
  obtain ⟨A, R, c, hA1, hA0, hRdiag, hR, hcpsd, hcle⟩ := hNA p ‹NeZero p› hp h4 hpNA
  have hup : Commons.paleyLocTheta p hp.pos ≤ 2 + Real.sqrt ((p : ℝ) / 2 + 4 * c) :=
    paleyUpper p hp h4 hp5 A R c hA1 hA0 hRdiag hR hcpsd
  have hin : (p : ℝ) / 2 + 4 * c ≤ ((p : ℝ) / 2) * (1 + ε) := by nlinarith [hcle]
  have hsq1 : Real.sqrt ((p : ℝ) / 2 + 4 * c) ≤ s * (1 + ε / 2) := by
    have h1 : Real.sqrt ((p : ℝ) / 2 + 4 * c) ≤ Real.sqrt (((p : ℝ) / 2) * (1 + ε)) :=
      Real.sqrt_le_sqrt hin
    have h2 : Real.sqrt (((p : ℝ) / 2) * (1 + ε)) = s * Real.sqrt (1 + ε) := by
      rw [hs, ← Real.sqrt_mul (by linarith : (0:ℝ) ≤ (p:ℝ)/2)]
    have h3 : Real.sqrt (1 + ε) ≤ 1 + ε / 2 := by
      have hnn : (0:ℝ) ≤ 1 + ε / 2 := by linarith
      have := Real.sqrt_le_sqrt (show (1 : ℝ) + ε ≤ (1 + ε / 2) ^ 2 by nlinarith)
      rwa [Real.sqrt_sq hnn] at this
    calc Real.sqrt ((p : ℝ) / 2 + 4 * c) ≤ s * Real.sqrt (1 + ε) := by rw [← h2]; exact h1
      _ ≤ s * (1 + ε / 2) := by nlinarith [hspos]
  have hss : s * s = (p : ℝ) / 2 := by rw [← sq]; exact hs2
  have hslarge : 4 / ε < s := by
    have h1 : (4 / ε) ^ 2 < (p : ℝ) / 2 := by
      have hh : (32 : ℝ) < (p : ℝ) * ε ^ 2 := by
        rw [div_lt_iff₀ (by positivity : (0:ℝ) < ε ^ 2)] at hP1; linarith
      rw [div_pow, div_lt_iff₀ (by positivity : (0:ℝ) < ε ^ 2)]
      nlinarith
    have h2 : Real.sqrt ((4 / ε) ^ 2) < Real.sqrt ((p : ℝ) / 2) :=
      Real.sqrt_lt_sqrt (sq_nonneg _) h1
    rwa [Real.sqrt_sq (by positivity : (0:ℝ) ≤ 4 / ε), ← hs] at h2
  have h2s : 2 / s < ε / 2 := by
    rw [div_lt_iff₀ hspos]
    have hmul : ε / 2 * (4 / ε) < ε / 2 * s := by
      apply mul_lt_mul_of_pos_left hslarge (by linarith)
    have hval : ε / 2 * (4 / ε) = 2 := by field_simp; ring
    linarith [hmul, hval.ge, hval.le]
  have hupper : Commons.paleyLocTheta p hp.pos / s - 1 < ε := by
    have hb : Commons.paleyLocTheta p hp.pos ≤ 2 + s * (1 + ε / 2) := by linarith [hup, hsq1]
    have hd : Commons.paleyLocTheta p hp.pos / s ≤ (2 + s * (1 + ε / 2)) / s := by
      rw [div_le_div_iff₀ hspos hspos]; nlinarith [hb, hspos]
    have he : (2 + s * (1 + ε / 2)) / s = 2 / s + (1 + ε / 2) := by field_simp
    rw [he] at hd
    linarith [h2s, hd]
  -- lower
  obtain ⟨Ybar, θ, hθpos, hYd, hYn, hYpsd, hθle⟩ := hNB p ‹NeZero p› hp h4 hpNB
  have hlo : ((p : ℝ) - 1) / 2 / θ ≤ Commons.paleyLocTheta p hp.pos :=
    paleyLower p hp h4 hp5 Ybar θ hθpos hYd hYn hYpsd
  have hden : (0 : ℝ) < (1 + ε / 4) * s := by positivity
  have hθub : θ ≤ (1 + ε / 4) * s := by rw [hs]; exact hθle
  have hstep : ((p : ℝ) - 1) / 2 / ((1 + ε / 4) * s) ≤ ((p : ℝ) - 1) / 2 / θ := by
    rw [div_le_div_iff₀ hden hθpos]
    nlinarith [hθub, hPpos]
  have hkey : (1 - ε) * s < ((p : ℝ) - 1) / 2 / ((1 + ε / 4) * s) := by
    rw [lt_div_iff₀ hden]
    have hpe : (4 : ℝ) < (p : ℝ) * (3 * ε) := by
      rw [div_lt_iff₀ (by positivity : (0:ℝ) < 3 * ε)] at hP2; linarith
    nlinarith [hss, hspos, hε, hpe, hPpos]
  have hlower : 1 - ε < Commons.paleyLocTheta p hp.pos / s := by
    rw [lt_div_iff₀ hspos]
    linarith [hkey, hstep, hlo]
  rw [abs_lt]
  constructor
  · linarith
  · exact hupper
...
end Submissions.PaleyLocThetaFromCertificates.WoshuaJolk
