...
lemma sigma_sub (hp4 : p % 4 = 1) :
    haveI : NeZero p := NeZero.of_pos (Fact.out : Nat.Prime p).pos
    ∑ u : Commons.PaleyLocV p, ∑ v : Commons.PaleyLocV p, chi p ((u : ZMod p) - (v : ZMod p))
      = -(((p:ℝ) - 1) / 2) := by
  haveI : NeZero p := NeZero.of_pos (Fact.out : Nat.Prime p).pos
  classical
...
theorem lowerSharp (hp4 : p % 4 = 1) :
  ...
  have hle : (∑ u : Commons.PaleyLocV p, ∑ v : Commons.PaleyLocV p, X u v)
      ≤ Commons.paleyLocTheta p (Fact.out : Nat.Prime p).pos := le_csSup (bdd hp4) hmem
  have hsigval : ∑ u : Commons.PaleyLocV p, ∑ v : Commons.PaleyLocV p,
      chi p ((u : ZMod p) - (v : ZMod p)) = -(((p:ℝ) - 1) / 2) := sigma_sub (p := p) hp4
  have hcard := card_eq (p := p) hp4
  rw [← hNdef] at hcard
  have hpN : (p:ℝ) = 2 * (N:ℝ) + 1 := by linarith
  have hval : ∑ u : Commons.PaleyLocV p, ∑ v : Commons.PaleyLocV p, X u v
      = (Real.sqrt p - 1 + ((p:ℝ) - 1) / 2) / (Real.sqrt p + 1) := by
    rw [hSval, hsigval, hc, hpN]
    have hNne : (N:ℝ) ≠ 0 := ne_of_gt hN0
    have hsne : Real.sqrt p + 1 ≠ 0 := ne_of_gt hsp
    field_simp
    ring
  rw [← hval]
  exact hle
...
theorem proof :
    ∀ p : ℕ, ∀ hp : Nat.Prime p, p % 4 = 1 →
      (Real.sqrt p - 1 + ((p : ℝ) - 1) / 2) / (Real.sqrt p + 1)
        ≤ Commons.paleyLocTheta p hp.pos := by
  intro p hp hp4
  haveI : Fact (Nat.Prime p) := ⟨hp⟩
  exact lowerSharp hp4

end Submissions.PaleyLocThetaLowerSharp.SharpPrimal
