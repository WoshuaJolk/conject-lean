...
/-!
# MUST-FAIL CONTROL: the conclusion with the non-orthogonality clause deleted

This is a deliberately weakened theorem submitted with `expect: red`. It proves the
`UPBDegreeThree224k` conclusion **without** the clause `<z_i, z_j> != 0`, which is the whole
content. If the verifier's anti-restatement bridge is doing its job this must red as
`restatement`; a green here would mean the label can be discharged by a strictly weaker claim.
...
namespace Submissions.UPBDegreeThree224k.MalloryWeakened
...
end Submissions.UPBDegreeThree224k.MalloryWeakened
