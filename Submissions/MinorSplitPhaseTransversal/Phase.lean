    ...
    ∀ (u : Fin n₁ → Fin k → ℂ) (w : Fin n₂ → Fin k → ℂ),
      ...
      ∃ z : Fin k → ℂ, IsPhase z ∧
        (∀ (i : Fin n₁) (j : Fin n₂),
          pair (u i) (scale z w j) ≠ 0) ∧
        Transversal u (scale z w) := by
  intro k n₁ n₂ hk u w hcross hminor
  have hcross' :
      ∀ (i : Fin n₁) (j : Fin n₂),
        ∃ z : Fin k → ℂ, IsPhase z ∧
          pair (u i) (scale z w j) ≠ 0 := by
    intro i j
    obtain ⟨r, hu, hw⟩ := hcross i j
    have hp : crossPoly u w i j ≠ 0 :=
      crossPoly_ne_zero_of_support u w i j r hu hw
...
end Submissions.MinorSplitPhaseTransversal.Phase
