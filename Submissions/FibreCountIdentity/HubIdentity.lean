...
theorem proof : ∀ (a b m : ℕ) (A B : Fin m → Finset ℕ) (T : ℕ → Finset (Fin m)),
    Commons.OneCrossSPS a b m A B →
    (∀ (e : ℕ) (j : Fin m), j ∈ T e ↔ e ∈ B j) →
    ∀ i : Fin m, (∑ e ∈ A i, (T e).card) + 1 = m := by
  intro a b m A B T h hT i
  have hTeq : ∀ e : ℕ, T e = Tset B e := by
    intro e
    ext j
    rw [hT e j, Tset, Finset.mem_filter]
    simp
  simp_rw [hTeq]
  exact fibre_identity h i

end Submissions.FibreCountIdentity.HubIdentity
