...
/-!
Proof of `Statements.MoulinOllagnierWitnessAtK22.statement`. Definitions re-declared
verbatim; a submission may not import `Statements.*`.
...
namespace Submissions.MoulinOllagnierWitnessAtK22.AlgProp
...
theorem proof :
  ...
  (pts.map (tau 22 F2)).map phi = (pts.map phi).map (tau 22 [2]) := by decide


end Submissions.MoulinOllagnierWitnessAtK22.AlgProp
